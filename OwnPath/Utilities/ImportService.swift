...
enum ImportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedFormat:
            return "Unsupported file format. Please use JSON, CSV, or Markdown files exported from OwnPath."
    ...
    static func detectDataType(from data: Data, format: ExportFormat) -> ImportDataType {
        ...
        case .markdown:
            if let text = String(data: data, encoding: .utf8) {
                if text.contains("# OwnPath Location Points Export") { return .locationPoints }
                if text.contains("# OwnPath Export") { return .visits }
            }
        }
        return .visits
    }
