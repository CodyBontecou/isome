...
struct OwnPathProvider: TimelineProvider {
    func placeholder(in context: Context) -> OwnPathEntry {
        OwnPathEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (OwnPathEntry) -> Void) {
        let data = SharedLocationData.load() ?? .empty
        let entry = OwnPathEntry(date: Date(), data: data)
        completion(entry)
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<OwnPathEntry>) -> Void) {
        let data = SharedLocationData.load() ?? .empty
        let entry = OwnPathEntry(date: Date(), data: data)
...
struct OwnPathEntry: TimelineEntry {
    let date: Date
...
struct OwnPathWatchWidget: Widget {
    let kind: String = "OwnPathWatchWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: OwnPathProvider()) { entry in
            OwnPathWidgetEntryView(entry: entry)
                .containerBackground(.fill.tertiary, for: .widget)
        }
        .configurationDisplayName("OwnPath")
...
struct OwnPathWidgetEntryView: View {
    @Environment(\.widgetFamily) var family
    let entry: OwnPathEntry
...
#Preview("Circular - Active", as: .accessoryCircular) {
    ...
    OwnPathEntry(date: .now, data: SharedLocationData(
        isTrackingEnabled: true,
