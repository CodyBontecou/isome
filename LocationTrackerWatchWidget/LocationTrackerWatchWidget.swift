...
struct LocationTrackerProvider: TimelineProvider {
    func placeholder(in context: Context) -> LocationTrackerEntry {
        LocationTrackerEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (LocationTrackerEntry) -> Void) {
        let data = SharedLocationData.load() ?? .empty
        let entry = LocationTrackerEntry(date: Date(), data: data)
        completion(entry)
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<LocationTrackerEntry>) -> Void) {
        let data = SharedLocationData.load() ?? .empty
        let entry = LocationTrackerEntry(date: Date(), data: data)
...
struct LocationTrackerEntry: TimelineEntry {
    let date: Date
...
struct LocationTrackerWatchWidget: Widget {
    let kind: String = "LocationTrackerWatchWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: LocationTrackerProvider()) { entry in
            LocationTrackerWidgetEntryView(entry: entry)
...
struct LocationTrackerWidgetEntryView: View {
    @Environment(\.widgetFamily) var family
    let entry: LocationTrackerEntry
...
#Preview("Circular - Active", as: .accessoryCircular) {
    LocationTrackerWatchWidget()
} timeline: {
    LocationTrackerEntry(date: .now, data: SharedLocationData(
        isTrackingEnabled: true,
